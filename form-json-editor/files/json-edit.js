(function ($) {
  $.fn.jsonEdit = function (options) {
    ...
    function initOptions() {
      options = options || {};
      options["expandingLevel"] =
        options["expandingLevel"] == null ? -1 : options["expandingLevel"]; // -1:expand all
      ...
      options["treeExpandCollapseButton"] =
        options["treeExpandCollapseButton"] || "true";
    }
    ...
    function initWidget() {
      ...
      initEvents();
      validateWidget();
      if (options["afterWidgetCreated"])
        options["afterWidgetCreated"](options["value"], options["schema"]);
    }
    ...
    function validateInput(elm) {
      var v_required = fixNU(elm.attr("data-required"), "false"),
        v_min = elm.attr("data-min"),
        v_max = elm.attr("data-max");

      v_min = v_min ? parseFloat(v_min) : null;
      v_max = v_max ? parseFloat(v_max) : null;

      if (
        elm.hasClass("j-input-text") ||
        elm.hasClass("j-input-textarea") ||
        elm.hasClass("j-input-select") ||
        elm.hasClass("j-input-number") ||
        elm.hasClass("j-input-date")
      ) {
        elm.attr(
          "data-is-valid",
          v_required == "true" && fixNU(elm.val(), "") == "" ? "false" : "true"
        );
        ...
        if (v_min) {
          if (
            ...
            elm.attr(
              "data-is-valid",
              elm.val().length < v_min ? "false" : "true"
            );
            if (elm.attr("data-is-valid") == "false") return;
          }
        ...
        if (v_max) {
          if (
            ...
            elm.attr(
              "data-is-valid",
              elm.val().length > v_max ? "false" : "true"
            );
            if (elm.attr("data-is-valid") == "false") return;
          }
      ...
      if (elm.hasClass("j-input-checkbox")) {
        elm.attr(
          "data-is-valid",
          v_required == "true" && elm.prop("checked") == false
            ? "false"
            : "true"
        );
      }
      ...
      if (elm.hasClass("j-input-html")) {
        ...
        elm
          .next(".j-input-html-div:first")
          .attr("data-is-valid", v_required == "true" && v ? "false" : "true");
      }
    }
    ...
    function initEvents() {
      ...
      renderPlace
        .find(".j-add-array-item")
        .off("click")
        .on("click", function () {
          addArrayItem($(this), true, null);
        });
      renderPlace
        .find(".j-remove-array-item")
        .off("click")
        .on("click", function () {
          removeArrayItem($(this));
        });
      renderPlace
        .find(".j-input-text,.j-input-textarea,.j-input-date,.j-input-number")
        .off("keyup")
        .on("keyup", function () {
          valueChanged($(this));
        });
      renderPlace
        .find(
          ".j-input-checkbox,.j-input-radio,.j-input-select,.j-input-color,.j-input-date,.j-input-number,.j-input-html"
        )
        .off("change")
        .on("change", function () {
          valueChanged($(this));
      ...
      renderPlace
        ...
        .off("focus")
        .on("focus", function () {
          $(this).parents("td:first").find(".j-input-html-div:first").focus();
        });
    }
    ...
    function addArrayItem(arrayContainer, needInitiations, itemIndex) {
      ...
      var dataTemplate = JSON.parse(
        JSON.stringify(arrayTemplates[tId]["dataTemplate"])
      );
      var dataPath = arrayContainer
        .parents("tr:first")
        .next()
        .find("td:first")
        .attr("data-path");
      if (
        V(options["value"], dataPath) == undefined ||
        V(options["value"], dataPath) == null
      ) {
    ...
    function valueChanged(changedObject) {
      ...
      if (
        changedObject.prop("tagName").toLowerCase() == "input" &&
        changedObject.prop("type").toLowerCase() == "checkbox"
      ) {
        p =
          p +
          "=" +
          (changedObject.prop("checked") == true ? "true" : "false") +
          ";";
      } else {
        p =
          p +
          "='" +
          (options["autoTrimValues"] == "true"
            ? jsonEscape(changedObject.val()).trim()
            : jsonEscape(changedObject.val())) +
          "';";
      }
      eval(p);
    ...
    function renderSchemaNode(schemaNode, schemaName, requiredItems) {
      ...
      if (
        ...
        return renderSimpleNode(
          schemaNode,
          schemaName,
          requiredItems ? requiredItems.includes(schemaName) : false
        );
    ...
    function renderSimpleNode(schemaNode, schemaName, isRequired) {
      var ContainerT =
        '<table $hover-hint$ class="j-container"><tr class="j-oject-value-row">$$$</tr></table>';
      var TitleT =
        '<td class="j-title-col">$$$</td><td class="j-sep-col"></td>';
      ...
      if (placeholderHint != "")
        placeholderHint = ' placeholder="' + placeholderHint + '" ';
      if (inlineHint != "")
        inlineHint = '<div class="j-inline-help">' + inlineHint + "</div>";
      if (validationHint != "")
        validationHint =
          '<div class="j-validation-help">' + validationHint + "</div>";
      if (isRequired) {
      ...
      if (nodeType == "boolean") {
        ...
        if (fixNU(schemaNode["enum"], "") == "") {
          ...
          if (editor == "html")
            htmlEditor =
              '<div class="j-input j-input-html-div" ' +
              requiredAtt +
              " contenteditable></div>";
          minAtt =
            fixNU(schemaNode["minLength"], "") +
            fixNU(schemaNode["minimum"], "");
          maxAtt =
            fixNU(schemaNode["maxLength"], "") +
            fixNU(schemaNode["maximum"], "");
          if (minAtt != "") minAtt = ' data-min="' + minAtt + '" ';
          if (maxAtt != "") maxAtt = ' data-max="' + maxAtt + '" ';
          if (editor == "textarea") {
            classAtt =
              ' class="j-input j-input-textarea' + additionalClass + '" ';
            inputBody =
              "<textarea " +
              classAtt +
              dataValueNameAtt +
            ...
            classAtt =
              ' class="j-input j-input-' + editor + additionalClass + '" ';
            inputBody =
              (editor == "color" ? "&nbsp;&nbsp;" : "") +
              '<input type="' +
              editor +
          ...
          if (editor == "radio") {
            ...
            inputBody =
              ...
              dataValueNameAtt +
              requiredAtt +
              ' /><span class="j-input-radio-label">null</span></label>&nbsp;&nbsp;&nbsp;';
            console.log(inputBody);
            jQuery.each(schemaNode["enum"], function (index) {
              inputBody +=
                '<label><input value="' +
                schemaNode["enum"][index] +
                '" type="radio" ' +
                classAtt +
                nameAtt +
                dataValueNameAtt +
                requiredAtt +
                ' /><span class="j-input-radio-label">' +
                schemaNode["enum"][index] +
                "</span></label>&nbsp;&nbsp;&nbsp;";
            });
          }
      ...
      TitleT = TitleT.replace(
        ...
        getSpacer(level) +
          getEC("") +
          getTitle(schemaNode, schemaName) +
          requiredStar
      );
    ...
    function renderObjectNode(schemaNode, schemaName) {
      ...
      var TitleT =
        '<tr class="j-oject-title-row"><td class="j-title-col">$$$</td><td class="j-sep-col"></td><td class="j-body-col">$inlinehint$</td></tr>';
      var childClass =
        options["expandingLevel"] != -1 && level + 1 > options["expandingLevel"]
          ? "j-collapsed"
          : "";
      ...
      var inlineHint = getUISetting(schemaNode, "inlineHint", "");
      if (inlineHint != "")
        inlineHint = '<span class="j-inline-help">' + inlineHint + "</span>";
      ...
      var BodyT =
        '<tr class="' +
        childClass +
        '"><td colspan="3" data-value-name="' +
        schemaName +
        '">$$$</td></tr>';
      TitleT = TitleT.replace(
        "$$$",
        getSpacer(level) + getEC(ecBtn) + getTitle(schemaNode, schemaName)
      );
      TitleT =
        options["renderFirstLevel"] == "false" && level == 0 ? "" : TitleT;

      level = level + 1;
      properties.forEach(function (item, index, arr) {
        temp += renderSchemaNode(
          schemaNode["properties"][item.toString()],
          item.toString(),
          schemaNode["required"]
        );
      });
    ...
    function renderArrayNode(schemaNode, schemaName) {
      var ContainerT = '<table class="j-container">$$$</table>';
      var TitleT =
        '<tr class="j-array-title-row"><td class="j-title-col">$$$&nbsp;$ArrTools$</td><td class="j-sep-col"></td><td class="j-body-col">$inlinehint$</td></tr>';
      var childClass =
        options["expandingLevel"] != -1 && level + 1 > options["expandingLevel"]
          ? "j-collapsed"
          : "";
      ...
      TitleT = TitleT.replace(
        "$ArrTools$",
        '<span class="j-add-array-item" data-array-loaded="false" data-template-id="' +
          itemTemplateId +
          '">[+]</span>'
      );
      TitleT = TitleT.replace("$inlinehint$", inlineHint);
      ...
      var itemDataTemplate = null,
        itemContainerT = null;
      var arrSchema = { title: "", type: arrType };

      level++;
      if (arrType == "string" || "number" || "boolean") {
        if (schemaNode["items"] && schemaNode["items"]["ui"])
          arrSchema["ui"] = schemaNode["items"]["ui"];
        if (schemaNode["items"] && schemaNode["items"]["enum"])
          arrSchema["enum"] = schemaNode["items"]["enum"];
        arrSchema["title"] =
          arrSchema["title"] +
          ' [$index$] <span class="j-remove-array-item" data-index="$index$"> X </span>';
        itemContainerT = renderSimpleNode(arrSchema, "$index$");
      }

      if (arrType.startsWith("#")) {
        var r =
          "['" + replaceAll(arrType.replace("#/", ""), "/", "']['") + "']";
        arrSchema = JSON.parse(JSON.stringify(V(options["schema"], r)));
        arrSchema["title"] =
          fixNU(arrSchema["title"], "") +
          ' [$index$] <span class="j-remove-array-item" data-index="$index$"> X </span>';
        itemContainerT = renderSchemaNode(arrSchema, "$index$");
      }
      level--;

      arrayTemplates[itemTemplateId] = {
        htmlTemplate: itemContainerT,
    ...
    function setValue(v) {
      ...
      renderPlace
        .find("input[data-path],select[data-path],textarea[data-path]")
        .each(function () {
          if (
            ...
            $(this).prop(
              "checked",
              V(v, $(this).attr("data-path")) == true ? true : false
            );
          } else if (
            $(this).prop("tagName").toLowerCase() == "input" &&
    ...
    function addArrayItemsToTheDOM() {
      ...
      arrayNodes.each(function () {
        ...
        var dataPath = addArrayItemBtn
          .parents("tr:first")
          .next("tr")
          .find("td:first")
          .attr("data-path");
        if (dataPath === undefined) {
          var o = addArrayItemBtn
            .parents("tr:first")
            .next("tr")
            .find("td:first");
          o.attr("data-path", generatePath(o));
          dataPath = addArrayItemBtn
            .parents("tr:first")
            .next("tr")
            .find("td:first")
            .attr("data-path");
    ...
    function getUISetting(schemaNode, name, ifNU) {
      return schemaNode["ui"] && schemaNode["ui"][name]
        ? schemaNode["ui"][name]
        : ifNU;
    }
    ...
    function getArrayType(schemaNode) {
      var type =
        schemaNode["items"] != undefined &&
        schemaNode["items"]["type"] != undefined
          ? schemaNode["items"]["type"]
          : "";
      if (type == "") {
        type =
          schemaNode["items"] != undefined &&
          schemaNode["items"]["$ref"] != undefined
            ? schemaNode["items"]["$ref"]
            : "string";
      }
      return type;
    }
    ...
    function ensureDataPath(dataPath) {
      ...
      pathParts.forEach(function (item, index, arr) {
        ...
        if (
          V(options["value"], pathCursor) === undefined ||
          V(options["value"], pathCursor) === null
        ) {
